...
- **`git wt merge` no longer treats the feature's own files as uncommitted changes on the target**: a `!wt` git alias from a linked worktree exports `GIT_DIR` pinned to that worktree's private gitdir. Worktree-local git (`status`, `read-tree`, `diff`, `rev-parse --git-dir`) still forwarded it, so the target conflict check compared the feature index to the main worktree and refused a merge that should have been clean. Those calls now discover the worktree from the directory `wt` chose; repo-level plumbing keeps the inherited context so the alias still finds the repository.
