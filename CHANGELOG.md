...
- **`wt config shell install fish` writes the wrapper where fish reads it under a custom `$XDG_CONFIG_HOME`**: the wrapper path was hardcoded to `~/.config/fish/functions/`, while the completion path resolved the XDG config directory — same tool, same command, two answers. With `$XDG_CONFIG_HOME` pointing anywhere but `~/.config`, install reported success for both files, fish autoloaded the completion and never saw the wrapper, so `wt switch` changed no directory and `wt config show` reported the integration as installed. The wrapper, the completion, and the directories `wt config shell uninstall` and `wt config show` scan now all come from one resolution of fish's `$__fish_config_dir`. On Unix an unset or default `$XDG_CONFIG_HOME` is unaffected; on Windows the fish completion moves off `%APPDATA%\fish\completions` onto the same path as the wrapper. The deprecated `conf.d` file install cleans up stays pinned to `~/.config/fish/conf.d`, the only place worktrunk ever wrote it.

- **Worktree safety checks no longer inherit Git status display filters**: `status.showUntrackedFiles = no` hid untracked files, while `submodule.<name>.ignore = all` hid dirty submodules, so `wt remove`, `wt merge`, or unattended `wt step prune` could discard work without `--force`. Safety checks now force both kinds of changes visible and name the path that blocks removal. Auto-staging warnings for `wt merge`, `wt step commit`, `wt step squash`, and `wt step relocate --commit` likewise override hidden-untracked settings, report the full count, and preview up to 10 terminal-safe paths without terminal-control or non-UTF-8 ambiguity. `wt merge --stage=tracked` considers tracked worktree changes, while `--stage=none` considers only the existing index; both leave untracked files alone when retaining the worktree, refuse residual changes before updating the target when cleanup will run, and detect staged submodule updates hidden by `diff.ignoreSubmodules`. Merge cleanup also rechecks immediately before the target ref update, so dirty submodules or changes created during approval cannot leave a partially updated merge.
