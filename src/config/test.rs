...
fn test_expand_template_curly_braces_without_variables() {
    ...
    let vars: HashMap<&str, &str> = HashMap::new();
    let result =
        expand_template("echo {}", &vars, ShellEscapeMode::Posix, &test.repo, "test").unwrap();
...
fn snapshot_complex_templates() {
    ...
    for (name, template, branch) in test_cases {
        ...
        let result =
            expand_template(template, &vars, ShellEscapeMode::Posix, &test.repo, "test").unwrap();
...
fn test_expand_template_literal_vs_escaped_unicode() {
    ...
    let literal_result = expand_template(
        template,
        &vars,
        ShellEscapeMode::Literal,
        &test.repo,
