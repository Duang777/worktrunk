...
pub fn stop_fsmonitor_daemon(worktree: &WorkingTree) {
    ...
    // Full scrub: under `git wt remove <other>` from a linked worktree, an
    // inherited `GIT_DIR` would send the stop IPC to the *invoking*
    // worktree's daemon and leave the target's running. Nothing here needs
    // a redirected object store or a `TempIndex`.
    let _ = Cmd::new("git")
