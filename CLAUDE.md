...
Git resolves `GIT_DIR`/`GIT_WORK_TREE` (and the rest of `INHERITED_GIT_PATH_VARS`) before walking up from the cwd, so an inherited value silently overrides a child's working directory. **Any spawn site that relocates a user command into a `wt`-chosen worktree — hooks, `wt step for-each`, the `--execute` program — must scrub these vars** (`Cmd::scrub_git_discovery_env` or `scrub_git_discovery_env_vars`). **Worktree-local plumbing** (`WorkingTree::run_command`, `TempIndex::command`) is a separate bucket: it relocates `wt`'s own git into a worktree `wt` already resolved, and hand-rolls removal of the selection vars (`GIT_DIR`, `GIT_WORK_TREE`, `GIT_COMMON_DIR`, and `GIT_INDEX_FILE` where it doesn't set its own) rather than calling the full scrub — `apply_common_settings` applies env-removes after env-sets, so the full helper would also strip `GIT_OBJECT_DIRECTORY` that `with_object_store_env` and `TempIndex` set. Children running in the user's own context (aliases, `commit.generation`) and repo-level plumbing (`Repository::run_command`) keep the inherited context (absolutized). Full site classification and rationale: `scrub_git_discovery_env_vars` in `src/shell_exec.rs`.
